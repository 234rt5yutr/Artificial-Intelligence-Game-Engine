...
namespace {
    ...
    bool ParseUint64(const std::string& token, uint64_t& outValue) {
        if (token.empty()) {
            return false;
        }
    ...
    bool ParseUint32(const std::string& token, uint32_t& outValue) {
        if (token.empty()) {
            return false;
        }
        const char* begin = token.data();
    ...
    bool AssetManifest::Load(const std::filesystem::path& path) {
        ...
        std::vector<ManifestEntry> loadedEntries;
        ...
        while (std::getline(file, line)) {
            ...
            if (!ParseUint64(parts[0], assetId) ||
                !ParseUint32(parts[1], rawType) ||
                !ParseUint64(parts[4], sourceHash) ||
                !ParseUint64(parts[5], cookedTimestamp)) {
                continue;
            }

            entry.AssetId = assetId;
            entry.Type = static_cast<AssetType>(rawType);
            entry.SourcePath = parts[2];
            entry.CookedPath = parts[3];
            entry.SourceHash = sourceHash;
            entry.CookedTimestamp = cookedTimestamp;

            if (parts.size() >= 7) {
                entry.AddressKey = parts[6];
            }
            if (parts.size() >= 8) {
                entry.BundleId = parts[7];
            }
            if (parts.size() >= 9 && !parts[8].empty()) {
                const std::vector<std::string> depTokens = SplitString(parts[8], ',');
                for (const std::string& depToken : depTokens) {
                    uint64_t dependencyId = 0;
                    if (ParseUint64(depToken, dependencyId)) {
    ...
    bool AssetManifest::Save(const std::filesystem::path& path) const {
        ...
        for (const auto& entry : m_Entries) {
            ...
            for (size_t i = 0; i < entry.Dependencies.size(); ++i) {
                ...
                dependencyStream << entry.Dependencies[i];
            }
